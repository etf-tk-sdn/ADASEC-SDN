...
//              Support for different data widths, clock periods and interfaces
...
(
    ...
    output logic [MAX_PACKET_NUMBER_WIDTH-1:0]    pktcount = 0,
	...
	if (reset == 1) begin
	    cnt_clk <= 64'd0;
            ...
            from_reader_avalon.data = '0;
            from_reader_avalon.empty = '0;
        end else begin
    ...
    begin
     if(from_reader_avalon.ready==1 || from_reader_axis.ready==1) begin
	    ...
	    if(first == 1) begin                                                //remember timestamps for the first packet
	        timestamp_usec_first = timestamp_usec;
                timestamp_sec_first = timestamp_sec;
                first = 0;
            end
            timestamp_usec_new = timestamp_usec - timestamp_usec_first;         //calculate shifted timestamp in micro/nanoseconds
            timestamp_sec_new = timestamp_sec - timestamp_sec_first;            //calculate shifted timestamp in seconds
		        

             $display("TIMESTAMP USEC", timestamp_usec_new);
              $display("TIMESTAMP SEC", timestamp_sec_new);
	    ...
	    if (r!=0) begin
            ...
            if (toNanos ==1) begin
                timestamp_new_ps = timestamp_sec_new*1e12+timestamp_usec_new*1e3;
            end else begin
    		timestamp_new_ps = timestamp_sec_new*1e12+timestamp_usec_new*1e6;
            end
	    ...
	    if (cnt_clk<timestamp_new_ps) begin   //if current value of clock counter is not larger than precalculated timestamp, don't start with new packet
			...
			if(SIGNAL_TYPE == "avalon") begin
                            from_reader_avalon.empty <= (capLen > bytes_data_out) ? 0 : bytes_data_out - capLen;
	                end else begin
				    ...
				    from_reader_axis.data[i*8+:8] <= capLen > i ? $fgetc(file) : 8'b0;
                            ...
                            if(SIGNAL_TYPE == "avalon") begin
		                from_reader_avalon.empty <= (actLen > bytes_data_out) ? 0 : bytes_data_out - actLen; 
                            end else begin
