...
// Description: Test/Simulation for the entire system, including 5 instances of pcap reader (representing 4 eth ports + cpu port),
//              adaptive encryptor with network packet classifier, and 5 instances of pcap writer. 
//              Input sample (merge_sample_filtered.pcap) include packets from 3 flows. The same sample is sent as the input to each port. 
//              Only one flow should be encrypted (only payload of these packets)  
//              Obtained samples confirmed that the results are as expected: two samples are empty, since none of the sent packets were intended for 
//              ports where these samples are captured; packets without defined rule are sent to cpu (thus they are written in _a.pcap file);
//              packets from other two flows are sent to specified ports as expected; packets from one of these flows are encrypted as it is defined
//              (only payload, the header remains the same), and from the other one remain the same, since encryption is not defined for that flow. 

module test_alt_e100;
...
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_reader4(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_reader_next(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_reader_next_1(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_reader_next_2(.clk(clk),.rst(rst));
...
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_cpu(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_readera(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_readerb(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) from_readerc(.clk(clk),.rst(rst));

avalon_if #(.DATA_WIDTH(DATA_WIDTH)) final_data_a(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) final_data_b(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) final_data_c(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) final_data_d(.clk(clk),.rst(rst));
avalon_if #(.DATA_WIDTH(DATA_WIDTH)) final_data_e(.clk(clk),.rst(rst));
...
pcapreader #(
        .PCAP_FILENAME( "merge_sample_filtered.pcap" ), 
...
pcapreader #(
	...
	.from_reader_avalon(from_reader1),
        .from_reader_axis(axis_int)
    );
...
pcapreader #(
	...
	.from_reader_avalon(from_reader2),
        .from_reader_axis(axis_int)
    );
...
pcapreader #(
	...
	.from_reader_avalon(from_reader3),
        .from_reader_axis(axis_int)
    );
...
pcapreader #(
	...
	.from_reader_avalon(from_reader4),
        .from_reader_axis(axis_int)
    );
...
rr_mux #(
   ...
   .from_cpu(from_reader),
   .from_eth_1(from_reader1),
   .from_eth_2(from_reader2),
   .from_eth_3(from_reader3),
   .from_eth_4(from_reader4),
   .to_sw(from_reader_next)
);
...
classifier #(
    ...
    .from_pcap_reader_to_classifier(from_reader_next),
    .from_classifier(from_reader_next_1)
);
...
avalon_fifo #(
      ...
      .clk(clk),
      .rst(rst),
      .input_avalon(from_reader_next_1),
      .output_avalon(from_reader_next_2),
      .depth_signal(depthh_fifo),
      .depth_signal2()
);
...
reader_demux readdemux(
    .clk(clk),
    .rst(rst),
    .from_pcap_reader(from_reader_next_2),
...
demux #(
   ...
   .from_sw(final_data1),
   .to_cpu(final_data_a),
   .to_eth_1(final_data_b),
   .to_eth_2(final_data_c),
   .to_eth_3(final_data_d),
   .to_eth_4(final_data_e)
);

pcapwriter #(
	.PCAP_FILENAME( "merge_sample_filtered_encrypted_a.pcap" ),
        .SIGNAL_TYPE(SIGNAL_TYPE),
        .CLOCK_PERIOD(2560),
        .DATA_WIDTH(DATA_WIDTH)
    ) pcapwr (
	...
	.to_writer_avalon(final_data_a),
        .to_writer_axis(axis_int)
    );

pcapwriter #(
	.PCAP_FILENAME( "merge_sample_filtered_encrypted_b.pcap" ),
        .SIGNAL_TYPE(SIGNAL_TYPE),
        .CLOCK_PERIOD(2560),
        .DATA_WIDTH(DATA_WIDTH)
    ) pcapwr1 (
	...
	.to_writer_avalon(final_data_b),
        .to_writer_axis(axis_int)
    );

pcapwriter #(
	.PCAP_FILENAME( "merge_sample_filtered_encrypted_c.pcap" ),
        .SIGNAL_TYPE(SIGNAL_TYPE),
        .CLOCK_PERIOD(2560),
        .DATA_WIDTH(DATA_WIDTH)
    ) pcapwr2 (
	...
	.to_writer_avalon(final_data_c),
        .to_writer_axis(axis_int)
    );

pcapwriter #(
	.PCAP_FILENAME( "merge_sample_filtered_encrypted_d.pcap" ),
        .SIGNAL_TYPE(SIGNAL_TYPE),
        .CLOCK_PERIOD(2560),
        .DATA_WIDTH(DATA_WIDTH)
    ) pcapwr3 (
	...
	.to_writer_avalon(final_data_d),
        .to_writer_axis(axis_int)
    );

pcapwriter #(
	.PCAP_FILENAME( "merge_sample_filtered_encrypted_e.pcap" ),
        .SIGNAL_TYPE(SIGNAL_TYPE),
        .CLOCK_PERIOD(2560),
        .DATA_WIDTH(DATA_WIDTH)
    ) pcapwr4 (
    ...
    initial begin
       // final_data.ready = 1;
        final_data_a.ready = 1;
        final_data_b.ready = 1;
        final_data_c.ready = 1;
        final_data_d.ready = 1;
        final_data_e.ready = 1;
...
//Flow ID: 0
        ...
        csr_writedata[23:16] = 8'(3);   //kofol prosljedjuje se na drugi ulaz - routing tag
