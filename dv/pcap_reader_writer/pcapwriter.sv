...
//              Support for different data widths, interfaces and clock periods
...
(
	...
	//$fwrite(file, "%u", pcap_buffsz32); // .snaplen - Max length of captured packets (65k bytes) //A: U primjeru saobracaja koji imam je 256 kB - 00 00 04 00
    ...
    begin
        if (to_writer_avalon.valid || to_writer_axis.valid) begin
            if(!first_sop) begin
	        if(to_writer_avalon.sop || to_writer_axis.data) begin   //Axis interface - first occurence of data signal
                    timestamp = cnt_clk/1000;     
                    first_sop = 1;
                end
	    end
	...
	//Packet data 
	   // if (to_writer_avalon.ready || to_writer_axis.ready) begin
		...
		if(SIGNAL_TYPE == "avalon") begin
                    pktSz = ((to_writer_avalon.eop) ? bytes_data_out - to_writer_avalon.empty : bytes_data_out) + pktSz;
                end else begin
	    ...
	    if(to_writer_avalon.eop || to_writer_axis.last) begin              
                timestamp2 = cnt_clk/1000;     
                //$display("TIMESTAMP EOP", timestamp2); 

                $display("TIMESTAMP SOP", timestamp);	
                $display("TIMESTAMP EOP", timestamp2); 	
                $display(pktSz);
