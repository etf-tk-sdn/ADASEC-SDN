...
module alt_e100s10_eth_4 #(
    ...
    output                  inhibit_rx_reset,
	 output 						PHY_TLKIT_ACCESS_phy,
    output                  inhibit_slip,
...
reg [2:0]	tx_mac_sclr_r	/* synthesis preserve_syn_only */;
reg [3:0]	tx_pcs_sclr_r	/* synthesis preserve_syn_only */;
reg [1:0]	rx_mac_sclr_r	/* synthesis preserve_syn_only */;
reg [1:0]	rx_pcs_sclr_r	/* synthesis preserve_syn_only */;

always @(posedge tx_clk)	tx_mac_sclr_r <= {3{tx_mac_sclr}};
always @(posedge tx_clk)	tx_pcs_sclr_r <= {4{tx_pcs_sclr}};
always @(posedge rx_clk)	rx_mac_sclr_r <= {2{rx_mac_sclr}};
always @(posedge rx_clk)	rx_pcs_sclr_r <= {2{rx_pcs_sclr}};
...
wire                      tpcs_dout_valid_0     = (enable_rsfec==1) ? (!tx_pcs_sclr_r[3]): tpcs_dout_valid;
...
reg [WORDS*66-1:0]       rx_data_in_pcs; 
reg                 rx_data_valid_pcs; 
  
wire    [66*4-1:0]        tpcs_dout;

//------------------------------------
//---add pipe for timing; linhua;
wire [4*66-1:0]	tx_data_out_r0;
reg [4*66-1:0]	tx_data_out_r1, tx_data_out_r2, tx_data_out_r3, tx_data_out_r4;
wire [3:0]	tx_data_valid_r0;
reg [3:0]	tx_data_valid_r1, tx_data_valid_r2, tx_data_valid_r3, tx_data_valid_r4;
reg [WORDS*66-1:0]	rx_data_in_r, rx_data_in_r2, rx_data_in_r3, rx_data_in_r4; 
reg [WORDS*66-1:0]	rx_data_in_fec_r2, rx_data_in_fec_r3, rx_data_in_fec_r4;
reg [3:0]		rx_data_valid_r, rx_data_valid_r2, rx_data_valid_r3, rx_data_valid_r4; 
reg [3:0]		rx_data_valid_fec_r2, rx_data_valid_fec_r3, rx_data_valid_fec_r4;
always @ (posedge rx_clk) begin
	rx_data_in_r	<= rx_data_in;
	rx_data_in_r2	<= rx_data_in_r;
	rx_data_in_r3	<= rx_data_in_r2;
	rx_data_in_r4	<= rx_data_in_r3;
	rx_data_in_fec_r2 <= rx_data_in_r;
	rx_data_in_fec_r3 <= rx_data_in_fec_r2;
	rx_data_in_fec_r4 <= rx_data_in_fec_r3;
	rx_data_valid_r	 <= rx_data_valid[3:0];
	rx_data_valid_r2 <= rx_data_valid_r;
	rx_data_valid_r3 <= rx_data_valid_r2;
	rx_data_valid_r4 <= rx_data_valid_r3;
	rx_data_valid_fec_r2 <= rx_data_valid_r;
	rx_data_valid_fec_r3 <= rx_data_valid_fec_r2;
	rx_data_valid_fec_r4 <= rx_data_valid_fec_r3;
end
...
wire [3:0] tx_dll_lock_r;
//reg [3:0] tx_data_valid_in_reg[2:0];
...
alt_e100s10_rs_rx  #(
       ...
       .arst                   (rx_pcs_sclr_r[0]),	           
       .l0_din_pma             (rx_data_in_fec_r4[4*66-1: 3*66]), 
       .l1_din_pma             (rx_data_in_fec_r4[3*66-1: 2*66]),
       .l2_din_pma             (rx_data_in_fec_r4[2*66-1: 1*66]),
       .l3_din_pma             (rx_data_in_fec_r4[1*66-1: 0*66]),
       .lanes_valid_in_pma     (rx_data_valid_fec_r4),            
//	// to pcs
...
alt_e100s10_rs_tx  #(
      ...
      .arst         (tx_pcs_sclr_r[0]),		//reset contoller
       //   from tx pcs
...
always @ (posedge rx_clk) begin
	rx_data_in_pcs         <= (enable_rsfec == 1) ? caui4_rs_dout[4*66-1:0]                      : rx_data_in_r4;
	rx_data_valid_pcs      <= (enable_rsfec == 1) ? caui4_rs_dout[4*66+1] & !caui4_rs_dout[4*66] : &rx_data_valid_r4[3:0];
end
...
//wire [263:0] data_in;
//assign data_in = tx_data_out_rsfec;
...
if (ENABLE_ANLT == 1) begin: WITH_KR 	
  alt_e100s10_data_grp_mx_pipe  #(
    ...
    .data_1     (264'b0),
    .data_2     (tx_data_kr),
    .data_out   (tx_data_out_r0),
    .sel        (kr_mode),
    .data_valid_in (tx_dll_lock),//(tpcs_dout_valid_0),
    .data_valid_out (tx_dll_lock_r),//(tx_data_valid_r),
    .tx_pcs_sclr    (tx_pcs_sclr_r[2])
  );
...
always @ (posedge rx_clk) begin
	rx_data_in_pcs    <= rx_data_in_r4;
	rx_data_valid_pcs <= &rx_data_valid_r4[3:0];
end
...
assign    avmm_rs_rx_dval   = rs_rx_dval;
 ...
 if (ENABLE_ANLT == 1) begin: WITH_KR 	
  ...
  assign tx_data_out_r0   = tpcs_dout;
  assign tx_dll_lock_r = tx_dll_lock;
end
end
...
always @ (posedge tx_clk) begin
	tx_data_out_r1 <= tx_data_out_r0;
	tx_data_out_r2 <= tx_data_out_r1;
	tx_data_out_r3 <= tx_data_out_r2;
	tx_data_out_r4 <= tx_data_out_r3;
	tx_data_valid_r1 <= tx_data_valid_r0;
	tx_data_valid_r2 <= tx_data_valid_r1;
	tx_data_valid_r3 <= tx_data_valid_r2;
	tx_data_valid_r4 <= tx_data_valid_r3;
end

assign tx_data_out   = tx_data_out_r4;
assign tx_data_valid = tx_data_valid_r4;
...
alt_e100s10_pcs_t #(
    ...
    .SYNOPT_C4_RSFEC_DYN (SYNOPT_C4_RSFEC_DYN),
    .ENABLE_ANLT         (ENABLE_ANLT),
    .DEVICE_FAMILY       ("Stratix_10")
) tpcs  (
    .clk                (tx_clk),
    .reset              (tx_pcs_sclr_r[1]),
    .din_d              (pcs_din_d),
    .din_c              (pcs_din_c),
    .din_am             (tx_am_pcs),
    .dout               (tpcs_dout),
...
alt_e100s10_pcs_r  #(
    ...
    .clk                (rx_clk),
    .reset              (rx_pcs_sclr_r[1]),
    .xcvr_sclr          (xcvr_rx_sclr),
    .rx_pcs_rst_req     (rpcs_rst_req),
    .enable_rsfec       (enable_rsfec),
    .din                (rx_data_in_pcs),	//either from rxfec or pma
    .din_valid          (rx_data_valid_pcs),	//either from rxfec or pma
    .din_req            (),
...
wire [5:0]  delay;
assign  soft_csr_rst = eio_sys_rst;
// MAC - CSR 
...
// =================  MAC  ================== //
// -------------------------------------------//
wire tx_clk_locked, rx_clk_locked;
alt_e100s10_sync1r2 tx_clk_sync_inst (
  .din_clk (avmm_clk),
  .din     (tx_clk_stable),
  .dout_clk(tx_clk),
  .dout    (tx_clk_locked)
...
alt_e100s10_sync1r2 rx_clk_sync_inst (
  .din_clk (avmm_clk),
...
alt_e100s10_mac #(
    ...
    // Resets 
    .tx_mac_sclr            (tx_mac_sclr_r[1]),
    .rx_mac_sclr            (rx_mac_sclr_r[1]),
    .reset_csr              (csr_reset), 
...
alt_e100s10_reset_controller reset_controler (
    ...
    .tx_dll_lock            (tx_dll_lock_r),
    .tx_data_valid_core     (tpcs_dout_valid_0),
    .tx_data_valid_pma      (tx_data_valid_r0)
);
