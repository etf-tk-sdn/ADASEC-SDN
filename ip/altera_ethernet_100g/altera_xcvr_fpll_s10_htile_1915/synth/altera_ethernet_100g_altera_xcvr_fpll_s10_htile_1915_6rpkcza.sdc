...
set split_qsys_output_name [split altera_ethernet_100g_altera_xcvr_fpll_s10_htile_1915_6rpkcza "_"]
...
source "${script_dir}/${top_inst_name}_ip_parameters_6rpkcza.tcl"
source "${script_dir}/alt_xcvr_fpll_helper_functions_6rpkcza.tcl"
...
# -                                                              - #
# ---------------------------------------------------------------- #
fpll_initialize_db_6rpkcza fpll_db_6rpkcza

# ---------------------------------------------------------------- #
# --- Set all the instances of this core                       --- #
# ---------------------------------------------------------------- #
set alt_xcvr_fpll_s10_instances [ dict keys $fpll_db_6rpkcza ]
...
foreach inst $alt_xcvr_fpll_s10_instances {
  ...
  dict set alt_xcvr_fpll_s10_pins $inst [dict get $fpll_db_6rpkcza $inst]
  ...
  for {set i 0} {$i < $profile_cnt} {incr i} {
    ...
    if {[dict exists $alt_xcvr_fpll_s10_pins $inst fpll_refclk_port]} {
      ...
      dict for {mode mode_clks} $profile_clocks {
        ...
        set list_of_clk_names [fpll_prepare_to_create_clocks_6rpkcza $inst $mode $mode_clks $profile_cnt $i $alt_xcvr_fpll_s10_pins $multiply_factor_dict $divide_factor_dict $all_fpll_profile_clocks_names]
